...
import DragDrop from "@/components/ui/icons/icon-dragdrop";
...
const LinkForm = () => {
  ...
  const [draggedItemId, setDraggedItemId] = useState<number | undefined>(
    undefined
  );
  ...
  const dragStart = (e: React.DragEvent, id: number) => {
    ...
    // const formattedData = JSON.stringify(formDataLocal![id]);

    // e.dataTransfer.setData("text/plain", formattedData);
    setDraggedItemId(id);
    // console.log("draggedItemId: ", draggedItemId)
  };

  const dragDrop = (e: React.DragEvent, id: number) => {
    e.preventDefault();
    // const draggedItemId
    console.log("draggedItemId - dragdrop: ", draggedItemId);
    //@ts-expect-error
    setFormDataLocal!((prevState) => {
      const targetItemIndex = id;
      const newArray = [...prevState];
      
      const itemToMove = newArray.splice(draggedItemId!, 1)[0];
      console.log("itemToMove: ", itemToMove);
      newArray.splice(targetItemIndex, 0, itemToMove);
  ...
  return (
    <form onSubmit={handleSubmit} id="myForm">
      {Array(formLinkCount)
        ...
        .map((_, id) => (
          <div
            className="bg-accent-foreground p-5 rounded-xl"
            key={id}
            onDragStart={(e) => {
              dragStart(e, id);
            }}
            onDragOver={(e) => allowDrag(e)}
            onDrop={(e) => dragDrop(e, id)}
            onDrag={() => {
          ...
          >
            <div className="flex justify-between items-center ">
              ...
              <p
                className="fs-body-M text-muted hover:cursor-pointer"
            ...
            {formDataLocal![id][0] ? (
              ...
              <Select onValueChange={(value) => handleSelectChange(value, id)}>
                <SelectTrigger className="">
                  <SelectValue placeholder="Dropdown Field Default" />
                </SelectTrigger>
                <SelectContent>
